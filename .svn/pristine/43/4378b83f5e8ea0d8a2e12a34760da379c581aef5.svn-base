...
'204203105')";
			...
			if(FromVersion<new Version("11.1.1.0")) {
				...
				if(DataConnection.DBtype==DatabaseType.MySql) {
					for(int i=0;i<table.Rows.Count;i++) {
						groupNum=PIn.Long(table.Rows[i]["UserGroupNum"].ToString());
						command="INSERT INTO grouppermission (UserGroupNum,PermType) "
							+"VALUES("+POut.Long(groupNum)+","+POut.Int((int)Permissions.ProcedureNote)+")";
						Db.NonQ32(command);
					}
				}
				else {//oracle
					for(int i=0;i<table.Rows.Count;i++) {
						groupNum=PIn.Long(table.Rows[i]["UserGroupNum"].ToString());
						command="INSERT INTO grouppermission (GroupPermNum,NewerDays,UserGroupNum,PermType) "
							+"VALUES((SELECT MAX(GroupPermNum)+1 FROM grouppermission),0,"+POut.Long(groupNum)+","+POut.Int((int)Permissions.ProcedureNote)+")";
						Db.NonQ32(command);
					}
				}
			...
			if(FromVersion<new Version("12.0.1.0")) {
				...
				if(tableClaimFormNum.Rows.Count==0) {
					...
					if(DataConnection.DBtype==DatabaseType.MySql) {
						...
						command="INSERT INTO claimform(ClaimFormNum,Description,IsHidden,FontName,FontSize,UniqueID,PrintImages,OffsetX,OffsetY) "+
							"VALUES ((SELECT MAX(ClaimFormNum)+1 FROM claimform),'1500',0,'Arial',9,'OD9',1,0,0)";
							claimFormNum=Db.NonQ(command,true);
					}
					command="INSERT INTO claimformitem (ClaimFormItemNum,ClaimFormNum,ImageFileName,FieldName,FormatString,XPos,YPos,Width,Height) "
		...
		private static void To12_1_1() {
			if(FromVersion<new Version("12.1.1.0")) {
				string command;
				if(DataConnection.DBtype==DatabaseType.MySql) {
				...
				command="UPDATE preference SET ValueString = '12.1.1.0' WHERE PrefName = 'DataBaseVersion'";
				Db.NonQ(command);
			}
			To12_1_7();
		}

		///<summary>Oracle compatible: 03/06/2012</summary>
